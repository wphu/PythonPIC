...
def test_PoissonSolver(debug=DEBUG):
    ...
    def plots():
        ...
        ax1.set_title("Field")
        ax1.plot(x, g.electric_field, "r-", label="Fourier {:4.2f}".format(
                 l2_norm(field, g.electric_field)))
        ax1.plot(x, field, "g-", label="Anal")
        ax2.set_title("Potential")
        ax2.plot(x, g.potential, "r-", label="Fourier {:4.2f}".format(
...
def test_PoissonSolver_complex(debug=DEBUG):
    ...
    energy_direct = 0.5 * (g.electric_field**2).sum() * (2 * np.pi)
    ...
    field_correct = np.isclose(g.electric_field, anal_field[indices_in_denser_grid]).all()
    potential_correct = np.isclose(g.potential, anal_potential[indices_in_denser_grid]).all()
...
def test_PoissonSolver_ramp(debug=DEBUG):
    ...
    def plots():
        ...
        ax2.plot(g.x, potential, "g-", label="Anal")
        for ax in axes:
            ax.grid()
            ax.legend()
        plt.show()
