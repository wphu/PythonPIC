...
class Solver:
    def __init__(self, solve_algorithm, initialiation_algorithm):
        ...
        # self.apply_bc = bc_algorithm


def solve_fourier(grid, neutralize=True):
    grid.electric_field[1:-1, 0], grid.energy_per_mode = algorithms_grid.PoissonSolver(
...
def solve_buneman(grid):
    grid.electric_field, grid.magnetic_field, grid.energy_per_mode = algorithms_grid.BunemanWaveSolver(
...
BunemanSolver = Solver(solve_buneman, solve_fourier_neutral)
